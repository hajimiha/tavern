...
const initialRelationships = Object.fromEntries(
  npcs.map((npc) => [npc.id, {
    affinity: npc.id === 'loran' ? 26 : npc.id === 'liuan' ? 18 : 8,
    stage: npc.id === 'loran' ? 'acquainted' : 'stranger',
    chattedToday: false,
    giftedToday: false,
    memoryTags: npc.id === 'loran' ? ['雨夜借伞', '接下第一份委托'] : [],
...
export const initialGameState: GameState = {
  ...
  money: 2480,
  skills: {
    fishing: { level: 2, experience: 34, nextLevel: 80 },
    farming: { level: 4, experience: 126, nextLevel: 180 },
    mining: { level: 3, experience: 82, nextLevel: 140 },
    combat: { level: 2, experience: 46, nextLevel: 100 },
    magic: { level: 3, experience: 95, nextLevel: 150 },
  },
  stats: { health: 28, maxHealth: 28, attack: 7, mana: 18, maxMana: 18, magicDamage: 9 },
  inventory: {
    'moon-radish': 4,
    'mist-bean': 2,
    'moss-fertilizer': 3,
  ...
  mine: { currentFloor: 1, highestFloor: 7, unlockedElevators: [5] },
