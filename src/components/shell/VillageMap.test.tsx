...
describe('连续像素村庄地图', () => {
  ...
  it('提供可见的地图移动控件与键盘平移路径', async () => {
    const user = userEvent.setup()
    render(<GameProvider><VillageMap /></GameProvider>)

    const viewport = screen.getByRole('application', { name: '可拖动的雾灯谷地图' })
    expect(screen.getByRole('button', { name: '向左移动地图' })).toHaveAttribute('id', 'map-pan-left')
  ...
  it('在空白处拖动后仍允许下一次主动选择地点', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '前往渔家' }))
