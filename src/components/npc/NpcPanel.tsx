...
export function NpcPanel({ npcId }: { npcId: string }) {
  ...
  const preferredGiftAffinity = scaleReward(14, state.rules.affinityMultiplier)
  ...
  return (
    <section className="npc-panel" role="dialog" aria-modal="false" aria-label={`${npc.name}互动面板`}>
      ...
      {subview === 'gift' && <div className="npc-subview"><button id={`npc-subview-back-gift-${npc.id}`} className="subview-back" type="button" onClick={() => setSubview('actions')}>返回互动</button><h3>挑选礼物</h3><p>她偏爱的礼物会获得 {preferredGiftAffinity} 点好感，赠礼消耗 {interactionCost} 点精力。</p>{giftItems.length ? giftItems.map((itemId) => <button id={`npc-gift-${npc.id}-${itemId}`} key={itemId} type="button" onClick={() => { dispatch({ type: 'GIVE_GIFT', npcId, itemId, affinity: 14 }); setSubview('actions') }}><strong>{shopItems.find((item) => item.id === itemId)?.name ?? itemDisplayNames[itemId] ?? '未鉴定礼物'}</strong><small>持有 {state.inventory[itemId]}</small></button>) : <div className="inline-warning">背包中没有她偏爱的礼物。</div>}</div>}
